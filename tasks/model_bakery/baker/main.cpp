...
int main(int argc, char* argv[])
{
  ...
  if (!ret) {
    debugPrint("Failed to parse glTF\n");
    return -1;
  }
      ...
      for (auto attrib : prim.attributes) {
        ...
        if (attrib.first == "TEXCOORD_0") {
          for (size_t elem = 0; elem < elems; ++elem) {
            ...
            debugPrint("normalized texcoord (", normalizedTexcoord[0], ", ", normalizedTexcoord[1], ", ", normalizedTexcoord[2]);
            debugPrint(" goes to index ", currentPosInBuffer + elem * 32, "\n");
  ...
  loader.WriteGltfSceneToFile(&model, savepath, false, false, true, false);
