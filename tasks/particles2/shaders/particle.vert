...
layout(set=0, binding=0) readonly buffer particleParams{
...
void main(void)
{
  ...
  color = vec4(vec3(1.0f), particle_params[gl_InstanceIndex].velocityAndTimeToLiveRelative.w);

  vec4 res;
  vec4 p = particle_params[gl_InstanceIndex].posAndAngle;
  ...
  float angle = particle_params[gl_InstanceIndex].posAndAngle.w;
