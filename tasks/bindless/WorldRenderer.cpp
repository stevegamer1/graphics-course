...
void WorldRenderer::refresh_textures(vk::CommandBuffer cmd_buf) {
  ...
  if (texturesDirty) {
    ...
    // Vulkan whines when the array in shader is 32 descriptors-long, but shorter here.
    // That's why I add 32 - images.size() fake descriptors.
    // A crutch? Yes. Works? Kinda. At least it helps to relax Etna's false positives of validity checks.
    // (I mean the `build\_deps\etna-src\etna\source\DescriptorSet.cpp:133-137`).
    // for (size_t i = 0; i < 32 - images.size(); ++i) {
    //   bindings.push_back(
    //     etna::Binding{
    //       0,
    //       placeholderTextureManager.textures[i].genBinding(
    //                                               defaultSampler.get(),
    //                                               vk::ImageLayout::eShaderReadOnlyOptimal
    //                                             ),
    //       static_cast<uint32_t>(bindings.size())
...

    {
      ...
      // layouts[0] = etna::get_context().getDescriptorSetLayouts().getVkLayout(texturesDescriptorSet.getLayoutId());
      // layouts[1] = etna::get_context().getDescriptorSetLayouts().getVkLayout(relemToTextureMapDescriptorSet.getLayoutId());
  ...
  texturesDescriptorSet = etna::create_descriptor_set(
